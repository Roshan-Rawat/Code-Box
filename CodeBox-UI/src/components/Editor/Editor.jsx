...
func main() {
  ...
  const createRequest = async () => {
    try {
      ...
      setExecuting(true); // Show loading spinner
      ...
      if (res.success && res.data) {
        const submissionId = res.data.split('/api/v1/results/')[1];
        console.log('Submission ID:', submissionId);
  
        // Polling function to check status
        const pollStatus = async (submissionId) => {
          const statusResponse = await fetch(`/api/v1/results/${submissionId}`);
          const statusRes = await statusResponse.json();
        
          if (statusRes.success) {
            let val = statusRes.data;
            if (typeof val == 'string'){
              val = JSON.parse(val);
            }
            const { status, output, error } = val;
            console.log(status, output)
        
            if (status === 'Success' || status === 'Queued' || status === 'Processing' || status === 'Failed') {
              setOutput(output);
              if (error) setOutput(error)
              if (status === 'Queued' || status === 'Processing') {
                setTimeout(() => pollStatus(submissionId), 3000); // Poll every 3 seconds
              } else {
                setExecuting(false); // Stop spinner
        ...
        pollStatus(submissionId); // Start polling
      } else {
